...
export function notifyUpdateWcli(): void {
  ...
  if (wcliUpdate.update) {
    const { latest, current } = wcliUpdate.update;
    warn(
      `当前wcli处于旧版本:${current},最新版本是:${latest}。您可以使用 wcli upgrade命令或者 npm upadte -g wcli来更新到最新版本`,
    );
  }
}
...
export function notifyUpdatePlugin(pluginCacheParams: PluginCacheParams): void {
  ...
  if (pluginUpdate.update) {
    const { latest, current } = pluginUpdate.update;
    warn(
      `当前插件-${pluginName}处于旧版本:${current},最新版本是:${latest}。您可以使用wcli plugin upgrade ${pluginName}来更新到最新版本`,
    );
  }
}
