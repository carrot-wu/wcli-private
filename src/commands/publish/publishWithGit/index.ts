...
export default async function publishFileWithGit(publishParams: PublishFileWithGitParams): Promise<string | void> {
  ...
  if (!publishGitDir) {
    ...
    return isGithubRepo(git)
      ? throwHandleError('github的发布需要填写本机发布git仓库的绝对路径')
      : publishFileWithGitlabCommit(publishParams);
  }
  // 检查当前路径是否git目录
  ...
  const removePromiseArray = fileList
    .filter((fileName) => fileName !== '.git')
    .map((fileName) => fse.remove(path.resolve(workDir, fileName)));
  // 进行删除
