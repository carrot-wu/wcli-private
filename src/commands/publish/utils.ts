...
async function checkPublishGitToken(plugin: string): Promise<string> {
  ...
  const hasTokenPluginArray: string[] = Object.keys(pluginCacheToken).filter(
    (pluginName) => pluginCacheToken[pluginName] && pluginCacheToken[pluginName] !== NOT_NEED_TOKEN,
  );

  const hasPluginTokenChoices = hasTokenPluginArray.map((token) => ({
    key: token,
    name: token,
    message: token,
  }));
...
export { checkPublishGitToken, getPublishGitToken };
